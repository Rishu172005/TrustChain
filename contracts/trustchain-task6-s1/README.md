...
# Task 6 — Smart Contract Technical Appendix (S1 piece)

**Goal:** appendix covering all smart contracts — purpose, key functions, gas cost per operation, test coverage %.

## What's new
- **`SMART_CONTRACT_APPENDIX.md`** — the actual Task 6 deliverable. Covers all 5 contracts (purpose, key functions + access control, gas-cost table, coverage) with a summary table at the end.
- **`hardhat.config.js`** — added `hardhat-gas-reporter` and `solidity-coverage` so you can generate the real numbers.

## Run it to fill in the appendix's numbers
```bash
npm install
npx hardhat test          # gas-reporter prints a gas table + writes gas-report.txt
npx hardhat coverage      # writes coverage/index.html + prints a % summary
```

Copy the results into the placeholder cells in `SMART_CONTRACT_APPENDIX.md`. If coverage isn't 100% on any contract, add the missing test case(s) — don't lower the target.

Same sandbox limitation as every task: this environment can't download the Solidity compiler, so none of this has actually been run on my end — the appendix structure and all 35 tests are ready, but the gas/coverage numbers need to come from your machine.

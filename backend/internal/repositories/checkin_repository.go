...
// FindByUser returns all check-ins created by a user, newest first.
func (r *CheckInRepository) FindByUser(ctx context.Context, userID string) ([]models.CheckIn, error) {
	uid, err := primitive.ObjectIDFromHex(userID)
	if err != nil {
		return nil, fmt.Errorf("invalid userId %q: %w", userID, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("querying checkins by user: %w", err)
	}
	defer cursor.Close(ctx)

	var checkins []models.CheckIn
	if err := cursor.All(ctx, &checkins); err != nil {
		return nil, fmt.Errorf("decoding user checkins: %w", err)
	}
	return checkins, nil
}

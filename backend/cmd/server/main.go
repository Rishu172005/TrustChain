...
func main() {
	...
	case "federated":
		recommendProvider = recommendation.NewFederatedLearningProvider("", poiRepo, reviewRepo, checkinRepo)
		recommendProviderName = "federated"
		log.Info().Msg("recommendation provider: federated")
	default:
	...
	transactionHandler := handlers.NewTransactionHandler(checkinRepo, reviewRepo, poiRepo, log)
	...
	v1 := router.Group("/api/v1")
	{
		...
		v1.GET("/transactions", transactionHandler.Get)
	}
